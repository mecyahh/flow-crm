...
export default function NotificationsBell() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition px-4 py-2 text-sm"
        title="Notifications"
      >
        🔔
        {unread > 0 && (
          <span className="absolute -top-2 -right-2 h-5 min-w-[20px] px-1 rounded-full bg-red-600 text-white text-[11px] flex items-center justify-center">
            {unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[360px] rounded-2xl border border-white/10 bg-[#0b0f1a]/90 backdrop-blur-xl shadow-2xl overflow-hidden z-50">
