...
export default function LeaderboardPage() {
 ...
 // Single source of truth for selected date range (YYYY-MM-DD|YYYY-MM-DD)
 ...
 // Canonical weekly range used by KPI cards + leaderboard table + Supabase fetch
 const selectedWeek = useMemo(() => {
   return getWeekRangeFromPickerValue(range)
 }, [range])


 const weekDays = useMemo(() => {
   const out: { key: string; label: string; isSunday: boolean }[] = []
   const d0 = new Date(selectedWeek.start)
   for (let i = 0; i < 7; i++) {
     const d = new Date(d0)
 ...
 useEffect(() => {
   ...
   ;(async () => {
     try {
       ...
       const startTs = selectedWeek.start.toISOString()
       const endTs = selectedWeek.end.toISOString()
 ...
 return (
   <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
     ...
     <div className="w-full min-w-0 px-4 py-6 md:px-10 md:py-10">
       ...
       {/* Top-left range selector (shared source for all leaderboard data) */}
       <div className="mb-6">
         <FlowRangePicker
           value={range}
       ...
       <div className="glass rounded-2xl border border-white/10 overflow-hidden">
         ...
         <div className="overflow-x-auto lg:overflow-x-visible">
           <table className="w-full text-sm">
             <thead className="text-[11px] tracking-wide uppercase text-white/55">
               <tr className="border-b border-white/10">
                 ...
                 {weekDays.map((d) => (
                   <th key={d.key} className="text-right px-4 py-3 whitespace-nowrap">
                     {d.label}
                   </th>
                 ))}
                   ...
                   return (
                     <tr key={r.uid} className="border-b border-white/10 hover:bg-white/5 transition">
                       ...
                       {weekDays.map((d) => {
                         ...
                         if (v <= 0) {
                           return (
                             <td key={d.key} className="px-4 py-4 text-right font-extrabold text-red-300 whitespace-nowrap tabular-nums">
                               0
                             </td>
                           )
                         }
                         return (
                           <td key={d.key} className="px-4 py-4 text-right font-semibold text-green-300 whitespace-nowrap tabular-nums">
                             ${formatMoney(v)}
                           </td>
                         )
                       })}
                       ...
                       <td className="px-6 py-4 text-right font-semibold text-green-300 whitespace-nowrap tabular-nums">
...
function MiniStat({ label, value }: { label: string; value: string }) {
 return (
   <div className="glass p-6">
     <p className="text-xs font-medium tracking-wide uppercase text-white/55">{label}</p>
...
function getWeekRangeFromPickerValue(value: string) {
 const { start, end } = parseRange(value)
 const todayMonday = startOfWeekMonday(new Date())


 // Fallback to current week if picker does not yet have a value.
 const rawStart = start ? new Date(start + 'T00:00:00') : todayMonday
 const rawEnd = end ? new Date(end + 'T23:59:59.999') : endOfWeekSunday(rawStart)


 // Force Monday -> Sunday for weekly leaderboard consistency.
 const monday = startOfWeekMonday(rawStart)
 const sunday = endOfWeekSunday(rawEnd)
